...
class SBModel(BaseModel):
    ...
    def forward(self):
        ...
        t_idx = torch.randint(T, size=[1], device=self.device).long().item()
        self.time_idx = torch.full((bs,), t_idx, device=self.device, dtype=torch.long)
        ...
        realt_time_idx = torch.full((self.realt.size(0),), t_idx, device=self.device, dtype=torch.long)
        ...
        if self.opt.phase == 'test':
            ...
            bs =  self.real.size(0)
            t_idx = torch.randint(T, size=[1], device=self.device).long().item()
            self.time_idx = torch.full((self.real_A.size(0),), t_idx, device=self.device, dtype=torch.long)
            self.timestep = times[self.time_idx]
            visuals = []
            with torch.no_grad():
                self.netG.eval()
