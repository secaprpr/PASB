...
def _torchvision_models():
    ...
    try:
        ...
        raise ImportError('torchvision is required for resnet/convnext pathology classifiers') from exc
    return models


def _weights_arg(weights_enum, use_pretrained):
    if not use_pretrained:
        return None
    return weights_enum.DEFAULT


def _adapt_resnet_input(model, input_nc):
    if input_nc == 3:
        return
    old_conv = model.conv1
    model.conv1 = nn.Conv2d(input_nc, old_conv.out_channels, kernel_size=old_conv.kernel_size,
                            stride=old_conv.stride, padding=old_conv.padding, bias=False)


def _adapt_convnext_input(model, input_nc):
    if input_nc == 3:
        return
    old_conv = model.features[0][0]
    model.features[0][0] = nn.Conv2d(input_nc, old_conv.out_channels, kernel_size=old_conv.kernel_size,
                                     stride=old_conv.stride, padding=old_conv.padding, bias=True)


def define_C(input_nc, ndf, netC, num_classes, init_type='normal', init_gain=0.02, gpu_ids=[],
             pretrained_backbone=False):
    """Create a pathology classifier used by PASB CDAL."""
    if netC == 'small':
        net = SmallPathologyClassifier(input_nc, ndf, num_classes)
        return init_net(net, init_type, init_gain, gpu_ids)

    models = _torchvision_models()
    if netC == 'resnet18':
        net = models.resnet18(weights=_weights_arg(models.ResNet18_Weights, pretrained_backbone))
        _adapt_resnet_input(net, input_nc)
        net.fc = nn.Linear(net.fc.in_features, num_classes)
    elif netC == 'resnet50':
        net = models.resnet50(weights=_weights_arg(models.ResNet50_Weights, pretrained_backbone))
        _adapt_resnet_input(net, input_nc)
        net.fc = nn.Linear(net.fc.in_features, num_classes)
    elif netC == 'convnext_tiny':
        net = models.convnext_tiny(weights=_weights_arg(models.ConvNeXt_Tiny_Weights, pretrained_backbone))
        _adapt_convnext_input(net, input_nc)
        net.classifier[2] = nn.Linear(net.classifier[2].in_features, num_classes)
    else:
        raise NotImplementedError('Pathology classifier name [%s] is not recognized' % netC)
...
# Classes
##############################################################################
class SmallPathologyClassifier(nn.Module):
    """A compact IHC prior classifier used by CDAL and smoke tests."""

    def __init__(self, input_nc=3, ndf=64, num_classes=4):
        super(SmallPathologyClassifier, self).__init__()
        ...
        self.classifier = nn.Linear(ndf * 4, num_classes)
